...
impl RetentionPolicy {
    pub fn new(name: String, data_type: String, retention_days: i64, archive_after_days: i64) -> Self {
        Self {
...
impl ArchivalService {
    ...
    pub async fn archive_data(
        ...
        let expires_at = policy.delete_after_days.map(|days| Utc::now() + Duration::days(days));
    ...
    pub fn query_archives(&self, query: ArchiveQuery) -> Result<Vec<ArchiveRecord>> {
        ...
        let mut results: Vec<ArchiveRecord> = archives
            .values()
            .filter(|archive| {
                // Filter by data type
